...
void rkv_list_keys_ult(hg_handle_t h)
{
    ...
    margo_instance_id mid = margo_hg_handle_list_keys_instance(h);
    CHECK_MID(mid, margo_hg_handle_list_keys_instance);

    const struct hg_info* info = margo_list_keys_info(h);
    rkv_provider_t provider = (rkv_provider_t)margo_registered_data(mid, info->id);
    CHECK_PROVIDER(provider);

    hret = margo_list_keys_input(h, &in);
    CHECK_HRET_OUT(hret, margo_list_keys_input);
    ...
    if(!in.packed) sizes_to_transfer += in.count*sizeof(size_t);
    ...
    auto from_keys = rkv::UserMem{ ptr, in.from_ksize };
    ...
    auto keys = rkv::UserMem{ ptr + keys_offset, in.keys_bufsize };
    ...
    if(in.packed) {
        out.ret = static_cast<rkv_return_t>(
            database->listKeysPacked(from_key, in.inclusive, prefix, in.count, keys, ksizes));
    } else {
        out.ret = static_cast<rkv_return_t>(
            database->listKeys(from_key, in.inclusive, prefix, in.count, keys, ksizes));
    }
