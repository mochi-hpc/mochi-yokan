...
extern "C" yk_return_t yk_collection_exists(yk_database_handle_t dbh,
                                            const char* name,
                                            int32_t mode,
                                            uint8_t* flag) {
    ...
    hret = margo_create(mid, dbh->addr, dbh->client->coll_exists_id, &handle);
